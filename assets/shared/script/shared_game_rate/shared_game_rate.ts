import { _decorator, Component, EventMouse, Node, NodeEventType, ScrollView, Size, Sprite, tween, Vec3, Widget } from 'cc';
import { EventAfter } from '../lib/event_after_callback';
import { Action } from '../action';

const { ccclass, property } = _decorator;

@ccclass('SharedGameRate')
export class SharedGameRate extends Component {
    ...
    private playEnterAction(){
        ...
        .call(()=>{
            self.spBg.getComponent(Widget).enabled = true;
            self.ndRoot.active = true;
            Action.grayLayerFadeIn(self.ndRoot.getComponent(Sprite), 0.4)
        })
        .start()
    }
