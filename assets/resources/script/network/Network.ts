...
export class Network 
{
    ...
    public WEBSOCKET_ADDR = "wss://game2.wu6jv3.com/games/20600/1";
    ...
    public CreateWS() {
        ...
        url = (url == "" || url == undefined) ? this.WEBSOCKET_ADDR : (url + NativeClass.GetAutoEnterGame() + '/1');
    ...
    private OnOpen(event) {
        ...
        this.SendPing();
        EventCenter.getIns().emit("onConnect");
    ...
    private ResetWS()
    {
        ...
        if (this.mForceClose == false) 
        {
            if(this.mConnedTimes>3){
                let tself = this;
                this.showAlert(DataManager.getL10n("shared_login_fail_net_error_title"), DataManager.getL10n("shared_NetworkErrorMessage"),"","",()=>{
    ...
    {
        console.warn("SendCmd",MainCmdID,SubCmdID);
    ...
    private OnMessage(event) 
    {
        ...
        if (head.wMainCmdID.value == KN_CMD.MDM_KN_COMMAND) {
            ...
            if(head.wMainCmdID.value == roomCmd.MDM_GR_LOGON && head.wSubCmdID.value == roomCmd.SUB_GR_LOGON_SUCCESS){
                ...
                EventCenter.getIns().emit("betResultErr",[terr.nResult.value])
               ...
               console.log("1111");
                ...
                this.ClearWS();
                let tself = this;
               this.showAlert(DataManager.getL10n("shared_login_fail_account_error_title"),DataManager.getL10n("'-------------------'"),"","",()=>{
                    window["SystemEx"].restart();
                    // tself.ClearWS(false);
               },()=>{
                    window["SystemEx"].gameEnd();
        ...
        EventCenter.getIns().emit(eventName, [head.wMainCmdID.value, head.wSubCmdID.value, event.data, event.data.byteLength - 8]);
    }
    ...
    private showAlert(title,content,btnLabel1:string="",btnLabel2:string="",btn1Call=null,btn2Call=null){
        let tlabel1=btnLabel1?btnLabel1:DataManager.getL10n("shared_title_reload");
        let tlabel2=btnLabel2?btnLabel2:DataManager.getL10n("shared_btn_exit");
        assetManager.getBundle("shared").load("prefab/commom/shared_notice_box", Prefab, (err: Error, data: Prefab) => {
                if (err != null) {
                    return;
                }
                let nd = instantiate(data);
                nd.parent = director.getScene().children[0];
                let noticeBox = nd.getComponent("SharedNoticeBox") as any; // SharedNoticeBox
                noticeBox.setInfo(title, content, tlabel1, tlabel2);
                noticeBox.setClickCallback(()=>{if(btn1Call)btn1Call()},()=>{if(btn2Call)btn2Call()});
