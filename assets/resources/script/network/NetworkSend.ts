...
export class NetworkSend
{
    ...
    public sendStartSpinPromise(betConfig){
        ...
        return new Promise((resolve, reject)=> {
            ...
            let tsucess = function(){
                let responeData = DataManager.betResult as TProtoGoldenCityBetRsp
                resolve(responeData)
    ...
    public buyFree(bet,round){
        let cmd = new roomCmd.Cmd_C_Free();
        cmd.iBet.value = bet;
        cmd.iRound.value = round;
        Network.Instance.SendCmd(roomCmd.MDM_GR_USER, roomCmd.SUB_GR_BUY_FREE_RELL, cmd);
    }
